...
import { ChevronLeft, Scale, Users, FileText, Anchor, BookOpen, Clock, Fingerprint, Compass, Zap, Search, Printer, TriangleAlert, Building2, Eye, MapPin, Target, Shield } from 'lucide-react';
...
const ScandalDetailScreen: React.FC<ScandalDetailScreenProps> = ({ scandalName, onClose }) => {
            ...
            <div className="max-w-6xl mx-auto p-6 md:p-12 space-y-12">
                ...
                <div className="text-center space-y-6">
                    ...
                    <div className="flex items-center justify-center flex-wrap gap-4 text-sm font-bold uppercase tracking-widest text-stone-500 dark:text-stone-400">
                        ...
                        <span className="hidden sm:inline">•</span>
                        <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {data.country}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-red-50 dark:bg-red-900/10 p-6 rounded-2xl border border-red-200 dark:border-red-900/30 print:border-black print:bg-white">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-red-800 dark:text-red-400 border-b border-red-200 dark:border-red-900/30 pb-2"><Target className="w-4 h-4" /> The Core Issue</h3>
                            <p className="font-serif text-sm leading-relaxed text-red-900 dark:text-red-300">{data.coreIssue}</p>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2"><Users className="w-4 h-4" /> Key Figures Involved</h3>
                            <ul className="space-y-2 text-sm font-serif text-stone-600 dark:text-stone-300">
                                {data.keyFigures?.map((d: string, i: number) => <li key={i}>• {d}</li>)}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-8">
                        ...
                        <div className="bg-academic-paper dark:bg-stone-800/50 p-8 rounded-2xl border border-academic-line dark:border-stone-700 print:bg-stone-100 print:border-black">
                            <h3 className="text-2xl font-serif font-bold text-stone-900 dark:text-white mb-6 flex items-center gap-3"><TriangleAlert className="w-6 h-6 text-red-500" /> Historical Impact & Consequences</h3>
                            <p className="font-serif text-lg leading-relaxed text-stone-800 dark:text-stone-200 mb-6">{data.historicalImpact}</p>
                            
                            <h4 className="font-bold uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 mb-2 mt-6">Political & Legal Fallout</h4>
                            <p className="font-serif text-md leading-relaxed text-stone-700 dark:text-stone-300">{data.fallout}</p>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                             <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                                <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2"><Eye className="w-4 h-4" /> How It Was Exposed</h3>
                                <p className="text-sm font-serif text-stone-700 dark:text-stone-300 leading-relaxed">{data.exposure}</p>
                            </div>
                            <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                                <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2"><Shield className="w-4 h-4" /> Reforms & Outcomes</h3>
                                <ul className="space-y-3">
                                    {data.reforms?.length > 0 ? data.reforms.map((c: string, i: number) => (
                                        <li key={i} className="text-sm font-serif leading-relaxed">• {c}</li>
                                    )) : <li className="text-sm italic text-stone-500">No major reforms documented.</li>}
                                </ul>
                            </div>
                        </div>
                    </div>
