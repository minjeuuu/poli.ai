...
import { ChevronLeft, Lightbulb, MapPin, Target, Users, BookOpen, Clock, Compass, Zap, Printer, Building2, ShieldAlert } from 'lucide-react';
...
const ThinkTankDetailScreen: React.FC<ThinkTankDetailScreenProps> = ({ entityName, onClose }) => {
        ...
        return (
            <div className="h-full flex items-center justify-center bg-stone-50 dark:bg-stone-950">
                <div className="flex flex-col items-center gap-4 animate-pulse">
                    <Lightbulb className="w-12 h-12 text-yellow-500 animate-bounce" />
                    <p className="text-stone-500 dark:text-stone-400 font-serif font-bold uppercase tracking-widest text-sm">Briefing Papers...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="max-w-6xl mx-auto p-6 md:p-12 space-y-12">
                <div className="text-center space-y-6">
                    ...
                    <div className="flex items-center justify-center flex-wrap gap-4 text-sm font-bold uppercase tracking-widest text-stone-500 dark:text-stone-400">
                        ...
                        <span className="hidden sm:inline">•</span>
                        <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {data.headquarters}</span>
                    </div>
                </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2">
                                <Compass className="w-4 h-4" /> Alignment
                            </h3>
                            <p className="font-mono text-sm text-stone-700 dark:text-stone-300">{data.politicalAlignment}</p>
                        </div>
                        <div className="bg-yellow-50 dark:bg-yellow-900/10 p-6 rounded-2xl border border-yellow-200 dark:border-yellow-900/30 print:border-black print:bg-white print:border-solid">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-yellow-800 dark:text-yellow-400 border-b border-yellow-200 dark:border-yellow-900/30 pb-2">
                                <Target className="w-4 h-4" /> Key Focus Areas
                            </h3>
                            <ul className="space-y-3">
                                {data.keyFocusAreas?.map((d: string, i: number) => (
                                    <li key={i} className="text-sm font-serif text-yellow-900 dark:text-yellow-200">• {d}</li>
                                ))}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                            ...
                            <ul className="space-y-2 text-sm font-serif text-stone-600 dark:text-stone-300">
                                {data.keyPeople?.map((d: string, i: number) => <li key={i}>• {d}</li>)}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-8">
                        <div className="bg-academic-paper dark:bg-stone-800/50 p-8 rounded-2xl border border-academic-line dark:border-stone-700 print:bg-stone-100 print:border-black">
                            <h3 className="text-2xl font-serif font-bold text-stone-900 dark:text-white mb-6 flex items-center gap-3">
                                <Lightbulb className="w-6 h-6 text-stone-500" /> Institute Overview
                            </h3>
                            ...
                            <h4 className="font-bold uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 mb-2 mt-6 flex items-center gap-2">
                                <Zap className="w-4 h-4" /> Policy Influence & Impact
                            </h4>
                            <p className="font-serif text-md leading-relaxed text-stone-700 dark:text-stone-300 mb-6">{data.influence}</p>
                            
                            <h4 className="font-bold uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 mb-2 mt-6 flex items-center gap-2">
                                <BookOpen className="w-4 h-4" /> Notable Projects & Reports
                            </h4>
                            <ul className="space-y-2 mt-3">
                                {data.notableProjects?.map((proj: string, i: number) => (
                                    <li key={i} className="bg-stone-100 dark:bg-stone-800 p-3 rounded-lg text-sm font-serif text-stone-700 dark:text-stone-300">
                                        {proj}
                                    </li>
                                ))}
                            </ul>
