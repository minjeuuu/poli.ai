...
const DisciplineDetailScreen: React.FC<DisciplineDetailScreenProps> = ({ 
  ...
  const handleDownloadSyllabus = async () => {
      ...
      try {
          ...
          doc.setFontSize(16);
          doc.text("POLI ACADEMIC SYLLABUS", 20, y);
          y += 10;
          
          doc.setFontSize(14);
          doc.text(`DISCIPLINE: ${safeName}`, 20, y);
          y += 10;
          doc.text("GENERATED BY: POLI", 20, y);
          y += 15;
          ...
          const lines = [
              ...
              "",
              "Key Questions:",
              ...(data.overview?.keyQuestions?.map(q => `- ${q}`) || []),
              "",
              "2. CORE THEORIES",
              ...(data.coreTheories?.map(t => `- ${t.name} (${t.year}): ${t.summary}`) || []),
              "",
              "3. RESEARCH METHODS",
              ...(data.methods?.map(m => `- ${m.name}: ${m.description}`) || []),
              "",
              "4. KEY SCHOLARS",
              ...(data.scholars?.map(s => `- ${s.name} (${s.period}): ${s.contribution}`) || []),
              "",
              "5. FOUNDATIONAL READINGS",
              ...(data.foundationalWorks?.map(w => `- "${w.title}" by ${w.author} (${w.year})`) || [])
          ];
