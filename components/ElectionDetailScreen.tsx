...
import { ChevronLeft, Scale, Users, FileText, Anchor, BookOpen, Clock, Fingerprint, Compass, Zap, Search, Printer, TriangleAlert, Building2, Eye, Flag, MapPin } from 'lucide-react';
...
const ElectionDetailScreen: React.FC<ElectionDetailScreenProps> = ({ electionName, onClose }) => {
        ...
        return (
            <div className="h-full flex items-center justify-center bg-stone-50 dark:bg-stone-950">
                <div className="flex flex-col items-center gap-4 animate-pulse">
                    <Flag className="w-12 h-12 text-blue-500 animate-bounce" />
                ...
                <div className="text-center space-y-6">
                    {data.imageUrl ? (
                        ...
                        <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-stone-200 dark:bg-stone-800 shadow-inner mb-4">
                            <Flag className="w-12 h-12 text-blue-600 dark:text-blue-400" />
                        </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-2xl border border-blue-200 dark:border-blue-900/30 print:border-black print:bg-white print:border-solid">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-blue-800 dark:text-blue-400 border-b border-blue-200 dark:border-blue-900/30 pb-2"><Users className="w-4 h-4" /> The Candidates</h3>
                            <ul className="space-y-4">
                                {data.candidates?.map((d: any, i: number) => (
                                    <li key={i} className="flex flex-col gap-1">
                                        <span className="font-serif text-lg font-bold">{d.name} {d.winner ? "🏆" : ""}</span>
                                        <span className="text-xs uppercase tracking-widest text-blue-600 dark:text-blue-300">{d.party}</span>
                                        <span className="font-mono text-sm text-stone-600 dark:text-stone-400">{d.votes}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                            <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2"><Zap className="w-4 h-4" /> Key Issues</h3>
                            <ul className="space-y-2 text-sm font-serif text-stone-600 dark:text-stone-300">
                                {data.keyIssues?.map((d: string, i: number) => <li key={i}>• {d}</li>)}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-8">
                        ...
                        <div className="bg-academic-paper dark:bg-stone-800/50 p-8 rounded-2xl border border-academic-line dark:border-stone-700 print:bg-stone-100 print:border-black">
                            <h3 className="text-2xl font-serif font-bold text-stone-900 dark:text-white mb-6 flex items-center gap-3"><Building2 className="w-6 h-6 text-stone-500" /> Significance & Geopolitics</h3>
                            <p className="font-serif text-lg leading-relaxed text-stone-800 dark:text-stone-200 mb-6">{data.historicalImpact}</p>
                            
                            <h4 className="font-bold uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 mb-2 mt-6">Aftermath</h4>
                            <p className="font-serif text-md leading-relaxed text-stone-700 dark:text-stone-300">{data.aftermath}</p>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                             <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                                <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-stone-500 dark:text-stone-400 border-b border-stone-100 dark:border-stone-800 pb-2"><Eye className="w-4 h-4" /> Political Context</h3>
                                <p className="text-sm font-serif text-stone-700 dark:text-stone-300 leading-relaxed">{data.context}</p>
                            </div>
                            <div className="bg-red-50 dark:bg-red-900/10 p-6 rounded-2xl border border-red-200 dark:border-red-900/30 print:border-red-500">
                                <h3 className="font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-xs text-red-800 dark:text-red-400 border-b border-red-200 dark:border-red-900/30 pb-2"><TriangleAlert className="w-4 h-4" /> Controversies & Scandals</h3>
                                <ul className="space-y-3">
                                    ...
                                    )) : <li className="text-sm italic text-red-900 dark:text-red-300">No major controversies documented.</li>}
                                </ul>
                            </div>
                        </div>
                    </div>
