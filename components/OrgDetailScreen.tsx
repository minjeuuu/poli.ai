  ...
  const handleDownloadDossier = async () => {
      ...
      try {
          ...
          if (data.keyActivities && data.keyActivities.length > 0) {
              sections.push({ title: "Key Activities", content: data.keyActivities });
          }
          if (data.analysis && data.analysis.strategicAnalysis) {
          ...
          generateAestheticPDF(
              data.name || orgName,
              data.type || "Organization Dossier",
              data.description || "No description provided.",
              sections,
              `${(data.name || orgName).replace(/\s+/g, '_')}_Dossier.pdf`
          );
      } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth pb-32 bg-stone-50/30 dark:bg-black/20">
          ...
          <div id="identity" ref={el => { sectionRefs.current['identity'] = el; }} className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl p-8 shadow-sm">
                  ...
                  <div className="mt-8 p-6 bg-academic-bg dark:bg-stone-800/30 border-l-4 border-academic-gold rounded-r-lg">
                      ...
                      <p className="font-serif text-lg leading-relaxed text-justify text-academic-text dark:text-stone-200 italic">"{data.mission}"</p>
                  </div>
          </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-500">
                  {Array.isArray(data.keyOrgans) && data.keyOrgans.map((organ, i) => (
                      ...
                      >
                          ...
                          <p className="text-xs text-stone-500 dark:text-stone-400 leading-relaxed text-justify font-serif">{organ.function}</p>
                      </button>
                  ))}
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {data.leadership.map((leader, i) => (
                          <div key={i} className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-6 rounded-xl shadow-sm">
                              ...
                              <p className="text-sm font-serif text-stone-600 dark:text-stone-400 leading-relaxed text-justify">{leader.bio}</p>
                          </div>
                      ))}
                  </div>
              </div>
                  ...
                  <div className="space-y-4">
                      {data.projects.map((project, i) => (
                          <div key={i} className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-6 rounded-xl shadow-sm hover:border-academic-accent dark:hover:border-indigo-500 transition-colors">
                              ...
                              <p className="text-sm font-serif text-stone-600 dark:text-stone-400 leading-relaxed text-justify">{project.description}</p>
                          </div>
                      ))}
                  </div>
              </div>
